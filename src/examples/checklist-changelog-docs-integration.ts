...
export async function runCompleteWorkflowExample(): Promise<void> {
  try {
    ...
    await orchestrator.registerAgent("checklist-agent", [
      "checkCompliance",
      "generateReport",
    ]);
    ...
    await orchestrator.registerAgent("documentation-agent", [
...
if (require.main === module) {
  runCompleteWorkflowExample()
    ...
    .catch((error: Error) => console.error("Example failed:", error));
}
